...
* ⚠️ *Tips* 有同学提到在终端执行 `gcc --version` 之后，直接弹出了 xcode 命令行工具的安装界面，那么就可以直接在那个弹出的界面**点击「安装」按钮**来安装，这样安装的是很小的（不超过 500MB）命令行工具，这就足够了，后面也不需要通过执行 `xcode-select --install` 来安装了；但如果点击「获取 Xcode」就会安装一个大小肯定超过 1GB 的 Xcode
